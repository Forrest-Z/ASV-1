...
% copy csv file name = 'Third';
path = '../data/';
Rtau = csvread(strcat(path, 'Balpha.csv'));
u = csvread(strcat(path, 'u.csv'));
alpha = csvread(strcat(path, 'alpha.csv'));
alpha_deg = csvread(strcat(path, 'alpha_deg.csv'));
dtau = csvread(strcat(path, 'tau.csv'));
rotation = csvread(strcat(path, 'rotation.csv'));

figure; 
subplot(311);
plot(Rtau(1, 2:end), '-r', 'linewidth', 2);
hold on;
plot(dtau(1, :), ':k', 'linewidth', 2);
ylabel('taux(N)');
legend('achieved force', 'desired force');
subplot(312); 
plot(Rtau(2, 2: end),'-r', 'linewidth', 2);
hold on;
plot(dtau(2, :), ':k', 'linewidth', 2);
ylabel('tauy(N)') 
legend('achieved force', 'desired force') 
subplot(313);
plot(Rtau(3, 2:end), '-r', 'linewidth', 2);
hold on;
